...
export const authRouter = router({
  // Get auth status
  getStatus: procedure.input(authProviderSchema).query(({ ctx, input }) => {
  ...
  isAuthenticated: procedure.input(authProviderSchema).query(({ ctx, input }) => {
    // Convert string to AuthProvider enum
    const provider = input as AuthProvider
  ...
  authenticate: procedure.input(authenticateSchema).mutation(async ({ ctx, input }) => {
    try {
      // Convert string to AuthProvider enum
  ...
  // Refresh token
  refreshToken: procedure.input(authProviderSchema).mutation(async ({ ctx, input }) => {
    try {
      // Convert string to AuthProvider enum
      const provider = input as AuthProvider
  ...
  // Revoke token
  revokeToken: procedure.input(authProviderSchema).mutation(async ({ ctx, input }) => {
    try {
      // Convert string to AuthProvider enum
      const provider = input as AuthProvider
  ...
  onStatusChange: procedure.input(authProviderSchema).subscription(({ ctx, input }) => {
