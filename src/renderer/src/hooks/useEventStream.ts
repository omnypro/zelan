...
export function useEventStream<T = unknown>(category: EventCategory, type?: string) {
  return useMemo(() => 
    createEventStream<T>(rendererEventBus, category, type).pipe(
 ...
 */
function accumulateEvents<T>(stream$: ReturnType<typeof useEventStream<T> | typeof useFilteredStream<T>>, limit: number) {
  return useMemo(() => stream$.pipe(
...
export function useLatestEvent<T = unknown>(
  ...
  const stream$ = useEventStream<T>(category, type)
  const data$ = useMemo(() => 
    stream$.pipe(map(event => event.data)),
  [stream$])
 ...
 */
export function useLatestFilteredEvent<T = unknown>(
  filterCriteria: EventFilterCriteria<T>,
  initialValue?: T
) {
  const stream$ = useFilteredStream<T>(filterCriteria)
