...
async fn test_adapter_lifecycle() -> Result<()> {
    ...
    assert!(
        ...
        "Adapter should be connected after connect()"
    );
    ...
    assert!(
        !adapter.is_connected(),
        "Adapter should be disconnected after disconnect()"
    );
...
async fn test_adapter_reconfiguration() -> Result<()> {
    ...
    let subscriber = Arc::new(crate::test_harness::TestSubscriber::new(
        "config_test",
        receiver,
    ));
    ...
    println!(
        "Before checking events - adapter is connected: {}",
        adapter.is_connected()
    );
    ...
    println!(
        "First phase: Collected {} events with current rate",
        event_count_slow
    );
    assert!(
        event_count_slow >= 1,
        "Should have at least 1 event with current rate, got {}",
        event_count_slow
    );
    ...
    let published = stats_json["events_published"].as_u64().unwrap_or(0);
    println!(
        "EventBus stats before reconfiguration: {} events published",
        published
    );
    ...
    println!(
        "MANUAL CHECK: Is adapter connected? {}",
        adapter.is_connected()
    );
    ...
    for i in 1..=15 {
        ...
        let published = stats_json["events_published"].as_u64().unwrap_or(0);
        println!(
            "EventBus stats after {}s: {} events published",
            i, published
        );
        ...
        if current_count >= 10 {
            println!(
                "Collected enough events ({}), continuing test",
                current_count
            );
            break;
        }
    ...
    println!(
        "Final event count: {} events with fast rate",
        event_count_fast
    );
    // For this test, count both initial events and manual events
    assert!(
        event_count_fast >= 3,
        "Should have 3+ events with fast rate, got {}",
        event_count_fast
    );
    ...
    assert!(
        manual_count > 0,
        "Should have received the manually published event"
    );
...
async fn test_multiple_adapters() -> Result<()> {
    ...
    assert!(
        sources.contains(&"adapter1"),
        "Should have events from adapter1"
    );
    assert!(
        sources.contains(&"adapter2"),
        "Should have events from adapter2"
    );
    assert!(
        sources.contains(&"adapter3"),
        "Should have events from adapter3"
    );
