use crate::error::{CircuitBreaker, ErrorCategory, ErrorRegistry, RetryPolicy, ZelanError, ZelanResult, with_retry};
        ...
        breaker
            .execute(|| async move {
                ...
                match f().await {
                    ...
                    Err(err) => {
                        // Register the error
                        let registered_err = this.error_registry.register(err, Some(operation_name)).await;
...
pub trait AdapterRecovery {
    ...
    where
        ...
        Fut: std::future::Future<Output = ZelanResult<T>> + Send,
    {
        self.recovery_manager().with_protection(operation_name, f).await
    }
...
mod tests {
    ...
    async fn test_circuit_breaker_opens_after_failures() {
        ...
        assert!(result.is_err());
        assert_eq!(counter.load(Ordering::SeqCst), 3);  // Should not have incremented
        ...
        assert!(result.is_ok());
        assert_eq!(counter.load(Ordering::SeqCst), 4);  // Should have incremented
    }
    ...
    async fn test_combined_protection() {
        ...
        assert!(result.is_err());
        assert_eq!(counter.load(Ordering::SeqCst), 9);  // Should not have incremented again
    }
}
