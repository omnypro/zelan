...
import { Event, EventCategory, isEvent } from '../../types/events';
...
export class EventBus implements IEventBus {
  ...
  public publish<T>(event: Event<T>): void {
    ...
    const baseEvent = event instanceof BaseEvent 
      ? event 
      : BaseEvent.fromObject(event);
  ...
  public ofCategory<T = unknown>(category: EventCategory): Observable<Event<T>> {
    return this.events.pipe(
      filter((event): event is Event<T> => event.category === category)
    );
  }
  ...
  public filter<T = unknown>(predicate: (event: Event) => boolean): Observable<Event<T>> {
    return this.events.pipe(
      filter((event): event is Event<T> => predicate(event))
    );
  }
  ...
  public payloads<T = unknown>(category: EventCategory, type: string): Observable<T> {
    return this.of<T>(category, type).pipe(
