...
impl WebSocketTestClient {
    ...
    pub async fn wait_for_json_message<F>(
        &self,
        predicate: F,
        timeout_ms: u64,
    ) -> Result<Option<Value>>
    ...
    pub async fn send_text(&self, text: &str) -> Result<()> {
        self.send_message(Message::Text(text.to_string().into()))
            .await
    }
    ...
    pub async fn send_ping(&self) -> Result<()> {
        ...
        let result1 = self
            .send_message(Message::Ping(vec![1, 2, 3, 4].into()))
            .await;
    ...
    pub async fn disconnect(&self) -> Result<()> {
        ...
        if let Some(stream) = stream_guard.take() {
            ...
            match tokio::time::timeout(Duration::from_secs(1), write.send(Message::Close(None)))
                .await
            {
                Ok(result) => {
                    if let Err(e) = result {
