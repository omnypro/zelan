...
import { useConfig, useFullConfig, useConfigChanges } from '../hooks/useConfig';
            ...
            <ul className="text-xs">
              {recentChanges.map((change, index) => (
                <li key={index} className="mb-2 pb-1 border-b border-gray-200 last:border-b-0">
                  <div><strong>Path:</strong> {change.key || '(root)'}</div>
                  <div><strong>Value:</strong> {JSON.stringify(change.value)}</div>
                  {change.previousValue !== undefined && (
                    <div><strong>Previous:</strong> {JSON.stringify(change.previousValue)}</div>
                  )}
                </li>
              ))}
            </ul>
