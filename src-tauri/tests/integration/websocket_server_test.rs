...
async fn test_basic_websocket_connection() -> Result<()> {
    ...
    assert_eq!(
        event["source"].as_str(),
        Some("test"),
        "Event source should be 'test'"
    );
    assert_eq!(
        event["event_type"].as_str(),
        Some("test.event"),
        "Event type should be 'test.event'"
    );
    assert!(
        event["payload"]["message"].as_str().is_some(),
...
async fn test_multiple_clients() -> Result<()> {
    ...
    let client1 =
        match tokio::time::timeout(Duration::from_secs(5), env.create_client("client1")).await {
            Ok(result) => result?,
    ...
    let client2 =
        match tokio::time::timeout(Duration::from_secs(5), env.create_client("client2")).await {
            Ok(result) => result?,
    ...
    let client3 =
        match tokio::time::timeout(Duration::from_secs(5), env.create_client("client3")).await {
            Ok(result) => result?,
...
async fn test_ping_pong() -> Result<()> {
    ...
    assert!(
        !messages.is_empty(),
        "Should have received some response after sending ping"
    );
...
async fn test_client_reconnection() -> Result<()> {
    ...
    // Send a test event
    env.publish_event("test", "before_disconnect", json!({"id": 1}))
        .await?;
    ...
    *connected_guard = false;
    ...
    env.publish_event("test", "during_disconnect", json!({"id": 2}))
        .await?;
    ...
    match tokio::time::timeout(Duration::from_secs(5), new_client.connect(env.ws_port)).await {
        Ok(result) => {
            if let Err(e) = result {
    ...
    env.publish_event("test", "after_reconnect", json!({"id": 3}))
        .await?;
    ...
    assert!(
        received.is_some(),
        "Client should have received event after reconnection"
    );

    // The client shouldn't have received the event sent during disconnection
    let messages = new_client.get_json_messages().await;
    let has_during_disconnect = messages
        .iter()
        .any(|msg| msg.get("event_type").and_then(|t| t.as_str()) == Some("during_disconnect"));

    assert!(
        !has_during_disconnect,
        "Client should not have received events sent during disconnection"
    );
...
async fn test_high_throughput() -> Result<()> {
    ...
    let received = env
        .wait_for_client_messages("throughput_client", EVENT_COUNT, 30000)
        .await?;
    assert!(
        received,
    ...
    assert_eq!(
        messages.len(),
        EVENT_COUNT,
        "Should have received exactly {} events",
        EVENT_COUNT
    );
