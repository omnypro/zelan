...
use zelan_lib::{flow::TraceContext, StreamEvent};
...
async fn test_basic_trace_flow() -> Result<()> {
    ...
    trace
        .add_span("initialize", "TestHarness")
        .context(Some(json!({
    ...
    assert!(
        receivers > 0,
        "Should have at least 1 receiver for the event"
    );
    ...
    assert_eq!(
        trace.event_type, "trace.test",
        "Trace event type should match"
    );

    // The path should include both the original span and the EventBus span
    assert!(
        trace.path.len() >= 2,
        "Trace path should have at least 2 entries"
    );
    assert!(
        trace.path.contains(&"TestHarness".to_string()),
        "Path should include TestHarness"
    );
    assert!(
        trace.path.contains(&"EventBus".to_string()),
        "Path should include EventBus"
    );

    // The spans should reflect the path
    assert!(trace.spans.len() >= 2, "There should be at least 2 spans");
    assert_eq!(
        trace.spans[0].name, "initialize",
        "First span should be 'initialize'"
    );
    assert_eq!(
        trace.spans[0].component, "TestHarness",
        "First span component should be 'TestHarness'"
    );
    assert_eq!(
        trace.spans[1].name, "publish",
        "Second span should be 'publish'"
    );
    assert_eq!(
        trace.spans[1].component, "EventBus",
        "Second span component should be 'EventBus'"
    );

    // Check the context in spans
    let test_harness_context = &trace.spans[0].context;
    assert!(
        test_harness_context.is_some(),
        "TestHarness span should have context"
    );
...
async fn test_multi_step_trace() -> Result<()> {
    ...
    trace3
        .add_span("initialize", "StreamManager")
        .context(Some(json!({
    ...
    println!(
        "Found {} traces in registry for multi_step_trace test",
        traces.len()
    );
    for (i, t) in traces.iter().enumerate() {
        println!(
            "Trace {}: source={}, event_type={}, path={:?}",
    ...
    if !system_traces.is_empty() {
        ...
        if !system_trace.spans.is_empty() {
            let has_clickhandler = system_trace
                .spans
                .iter()
                .any(|s| s.component == "ClickHandler");
            println!(
                "System trace has ClickHandler component: {}",
                has_clickhandler
            );
        }
    } else {
    ...
    let streamer_traces = traces
        .iter()
        .filter(|t| t.source == "streamer")
        .collect::<Vec<_>>();
    if !streamer_traces.is_empty() {
        let streamer_trace = &streamer_traces[0];
        // Check for specific spans if available
        if !streamer_trace.spans.is_empty() {
            let has_streammanager = streamer_trace
                .spans
                .iter()
                .any(|s| s.component == "StreamManager");
            println!(
                "Streamer trace has StreamManager component: {}",
                has_streammanager
            );
        }
    } else {
...
async fn test_adapter_trace_flow() -> Result<()> {
    ...
    for i in 1..=3 {
        ...
        trace.add_span("test", "TestHarness").context(Some(json!({
            "index": i,
