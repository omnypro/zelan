...
async fn test_auth_callback_preservation() -> Result<()> {
    ...
    // Register an auth callback with the original adapter that updates our counter
    {
        ...
        adapter
            .register_auth_callback(move |event| {
                ...
                println!(
                    "Auth callback invoked: {} times, event: {:?}",
                    current + 1,
                    event
                );
    ...
    adapter
        .trigger_auth_event(AuthEvent::DeviceCodeReceived {
    ...
    match &events[1] {
        ...
        other => panic!(
            "Second event should be AuthenticationSuccess, got {:?}",
            other
        ),
    }
    ...
    let independent_counter = Arc::new(AtomicUsize::new(0));
    {
        ...
        independent_adapter
            .register_auth_callback(move |_event| {
    ...
    assert_eq!(
        independent_counter.load(Ordering::SeqCst),
        0,
    ...
    independent_adapter
        .trigger_auth_event(AuthEvent::AuthenticationFailed {
...
async fn test_test_adapter_shared_state_preservation() -> Result<()> {
    ...
    assert_eq!(
        original_config.interval_ms, 15000,
    ...
    assert_eq!(
        original_config.generate_special_events, false,
...
async fn test_test_adapter_callback_preservation() -> Result<()> {
    ...
    // Register a callback with the original adapter that updates our counter
    {
        ...
        // Set up a test callback that will track invocations
        adapter
            .register_test_callback(move |event| {
                // Increment the counter
                let current = counter_clone.fetch_add(1, Ordering::SeqCst);
                println!(
                    "Test callback invoked: {} times, event: {:?}",
                    current + 1,
                    event.event_type()
                );
    ...
    adapter
        .trigger_event(TestEvent::Initial {
            counter: 1,
    ...
    cloned_adapter
        .trigger_event(TestEvent::Standard {
            counter: 2,
    ...
    assert_eq!(
        events[1].event_type(),
        "standard",
        "Second event should be Standard"
    );
...
async fn test_obs_adapter_callback_preservation() -> Result<()> {
    ...
    // Register a callback with the original adapter that updates our counter
    {
        ...
        adapter
            .register_obs_callback(move |event| {
                ...
                let current = counter_clone.fetch_add(1, Ordering::SeqCst);
                println!(
                    "OBS callback invoked: {} times, event: {:?}",
                    current + 1,
                    event.event_type()
                );
    ...
    adapter
        .trigger_event(ObsEvent::ConnectionChanged {
            connected: true,
            data: json!({"source": "original"}),
        })
        .await?;
    ...
    cloned_adapter
        .trigger_event(ObsEvent::SceneChanged {
    ...
    assert_eq!(
        events[0].event_type(),
        "connection_changed",
        "First event should be ConnectionChanged"
    );
    assert_eq!(
        events[1].event_type(),
        "scene_changed",
        "Second event should be SceneChanged"
    );
