...
export class WebSocketService {
   ...
   */
  private constructor(private eventBus: EventBus, private configStore: ConfigStore) {
  ...
  start(): boolean {
    ...
    try {
      ...
      this.server.on('error', (error) => {
        ...
        this.eventBus.publish(createSystemEvent(
  ...
  private sendRecentEvents(socket: WebSocketClient, params: any, requestId: string): void {
    ...
    const events = (this.eventBus as any).getRecentEvents?.({
  ...
  private broadcastEvent(event: BaseEvent): void {
    ...
    for (const client of this.clients) {
      ...
      if (client.filters) {
        ...
        if (client.filters.categories && 
            !client.filters.categories.includes(event.category)) {
          continue;
        }
        
        // Check type filter
        if (client.filters.types && 
            !client.filters.types.includes(event.type)) {
          continue;
        }
        
        // Check source filter
        if (client.filters.sources && 
            !client.filters.sources.includes(event.source.id)) {
          continue;
        }
      }
