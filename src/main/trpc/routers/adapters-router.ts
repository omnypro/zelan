...
export const adaptersRouter = router({
  ...
  // Get adapter by id
  getById: procedure.input(adapterIdSchema).query(({ ctx, input }: { ctx: TRPCContext; input: string }) => {
  ...
  // Create adapter
  create: procedure.input(adapterConfigSchema).mutation(async ({ ctx, input }) => {
    const adapter = await ctx.adapterManager.createAdapter(input)
    return createSerializableAdapter(adapter)
  }),

  // Start adapter
  start: procedure.input(adapterIdSchema).mutation(async ({ ctx, input }: { ctx: TRPCContext; input: string }) => {
    await ctx.adapterManager.startAdapter(input)
    return true
  }),

  // Stop adapter
  stop: procedure.input(adapterIdSchema).mutation(async ({ ctx, input }: { ctx: TRPCContext; input: string }) => {
  ...
  // Delete adapter
  delete: procedure.input(adapterIdSchema).mutation(async ({ ctx, input }: { ctx: TRPCContext; input: string }) => {
