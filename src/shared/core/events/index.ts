...
export class WebSocketClientDisconnectedEvent extends BaseEvent<{ clientId: string }> {
  constructor(clientId: string) {
    super('websocket', EventCategory.WEBSOCKET, WebSocketEventType.CLIENT_DISCONNECTED, { clientId });
  }
}

export class WebSocketMessageReceivedEvent extends BaseEvent<{ clientId: string; message: unknown }> {
  constructor(clientId: string, message: unknown) {
    super('websocket', EventCategory.WEBSOCKET, WebSocketEventType.MESSAGE_RECEIVED, { clientId, message });
  }
}

export class WebSocketMessageSentEvent extends BaseEvent<{ clientId: string; message: unknown }> {
  constructor(clientId: string, message: unknown) {
    super('websocket', EventCategory.WEBSOCKET, WebSocketEventType.MESSAGE_SENT, { clientId, message });
  }
}